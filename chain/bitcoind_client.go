...
//
// NOTE: This is part of the chain.Interface interface.
func (c *BitcoindClient) NotifyReceived(addrs []diviutil.Address) error {
	c.NotifyBlocks()
...
//	[]wire.OutPoint
//	[]*wire.OutPoint
//	map[wire.OutPoint]diviutil.Address
//	[]chainhash.Hash
//	[]*chainhash.Hash
func (c *BitcoindClient) LoadTxFilter(reset bool, filters ...interface{}) error {
	...
	for _, filter := range filters {
		...
		case []diviutil.Address, []wire.OutPoint, []*wire.OutPoint,
			map[wire.OutPoint]diviutil.Address, []chainhash.Hash,
			[]*chainhash.Hash:
...
func (c *BitcoindClient) Rescan(blockHash *chainhash.Hash,
	addresses []diviutil.Address, outPoints map[wire.OutPoint]diviutil.Address) error {
...
func (c *BitcoindClient) rescanHandler() {
	...
	for {
		...
		case update := <-c.rescanUpdate:
			...
			case map[wire.OutPoint]diviutil.Address:
				c.watchMtx.Lock()
				for op := range update {
					c.watchedOutPoints[op] = struct{}{}
				}
