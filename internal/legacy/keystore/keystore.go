...
import (
	...
	"github.com/DiviProject/diviwallet/internal/legacy/rename"
)
...
func pubkeyFromPrivkey(privkey []byte, compress bool) (pubkey []byte) {
	_, pk := diviec.PrivKeyFromBytes(diviec.S256(), privkey)
...
func chainedPrivKey(privkey, pubkey, chaincode []byte) ([]byte, error) {
	...
	case diviec.PubKeyBytesLenUncompressed, diviec.PubKeyBytesLenCompressed:
		// Correct length
	default:
...
func chainedPubKey(pubkey, chaincode []byte) ([]byte, error) {
	var compressed bool
	switch n := len(pubkey); n {
	case diviec.PubKeyBytesLenUncompressed:
		compressed = false
	case diviec.PubKeyBytesLenCompressed:
		compressed = true
	default:
	...
	oldPk, err := diviec.ParsePubKey(pubkey, diviec.S256())
	if err != nil {
		return nil, err
	}
	newX, newY := diviec.S256().ScalarMult(oldPk.X, oldPk.Y, xorbytes)
	if err != nil {
		return nil, err
	}
	newPk := &diviec.PublicKey{
		Curve: diviec.S256(),
		X:     newX,
		Y:     newY,
	}
...
func getAddressKey(addr diviutil.Address) addressKey {
...
type Store struct {
	...
	passphrase       []byte
	secret           []byte
	chainIdxMap      map[int64]diviutil.Address
...
func New(dir string, desc string, passphrase []byte, net *chaincfg.Params,
	...
	s := &Store{
		...
		chainIdxMap:      make(map[int64]diviutil.Address),
	...
	root, err := newRootDiviAddress(s, rootkey, nil, chaincode,
		createdAt)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ReadFrom(r io.Reader) (n int64, err error) {
	...
	s.chainIdxMap = make(map[int64]diviutil.Address)
...
func (s *Store) writeTo(w io.Writer) (n int64, err error) {
	...
	for _, wAddr := range s.addrMap {
		switch diviAddr := wAddr.(type) {
		case *diviAddress:
			e := &addrEntry{
				addr: *diviAddr,
			}
			copy(e.pubKeyHash160[:], diviAddr.AddrHash())
			if diviAddr.Imported() {
				// No order for imported addresses.
				importedAddrs = append(importedAddrs, e)
			} else if diviAddr.chainIndex >= 0 {
				// Chained addresses are sorted.  This is
				// kind of nice but probably isn't necessary.
				chainedAddrs[diviAddr.chainIndex] = e
			}
		...
		case *scriptAddress:
			...
			copy(e.scriptHash160[:], diviAddr.AddrHash())
...
func (s *Store) Lock() (err error) {
	...
	for _, addr := range s.addrMap {
		if baddr, ok := addr.(*diviAddress); ok {
			_ = baddr.lock()
		}
	}
...
func (s *Store) ChangePassphrase(new []byte) error {
	...
	for _, wa := range s.addrMap {
		// Only diviAddresses curently have private keys.
		a, ok := wa.(*diviAddress)
		if !ok {
			continue
		}
...
func (s *Store) NextChainedAddress(bs *BlockStamp) (diviutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) nextChainedAddress(bs *BlockStamp) (diviutil.Address, error) {
	addr, err := s.nextChainedDiviAddress(bs)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ChangeAddress(bs *BlockStamp) (diviutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) nextChainedDiviAddress(bs *BlockStamp) (*diviAddress, error) {
...
func (s *Store) LastChainedAddress() diviutil.Address {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) extendUnlocked(bs *BlockStamp) error {
	...
	newAddr, err := newDiviAddress(s, privkey, nil, bs, true)
	if err != nil {
		return err
	}
...
func (s *Store) extendLocked(bs *BlockStamp) error {
	...
	newaddr, err := newDiviAddressWithoutPrivkey(s, nextPubkey, nil, bs)
	if err != nil {
		return err
	}
	a = newaddr.Address()
...
func (s *Store) createMissingPrivateKeys() error {
	...
	prevAddr, ok := prevWAddr.(*diviAddress)
	if !ok {
		return errors.New("found non-pubkey chained address")
	}
...
func (s *Store) Address(a diviutil.Address) (WalletAddress, error) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	// Look up address by address hash.
	diviaddr, ok := s.addrMap[getAddressKey(a)]
	if !ok {
...
func (s *Store) SetSyncStatus(a diviutil.Address, ss SyncStatus) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ImportPrivateKey(wif *diviutil.WIF, bs *BlockStamp) (diviutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	pkh := diviutil.Hash160(wif.SerializePubKey())
	...
	diviaddr, err := newDiviAddress(s, privKey, nil, bs, wif.CompressPubKey)
	if err != nil {
		return nil, err
	}
	diviaddr.chainIndex = importedKeyChainIdx

	// Mark as unsynced if import height is below currently-synced
	// height.
	if len(s.recent.hashes) != 0 && bs.Height < s.recent.lastHeight {
		diviaddr.flags.unsynced = true
	}

	// Encrypt imported address with the derived AES key.
	if err = diviaddr.encrypt(s.secret); err != nil {
		return nil, err
	}

	addr := diviaddr.Address()
	...
	s.addrMap[getAddressKey(addr)] = diviaddr
	s.importedAddrs = append(s.importedAddrs, diviaddr)
...
func (s *Store) ImportScript(script []byte, bs *BlockStamp) (diviutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	if _, ok := s.addrMap[addressKey(diviutil.Hash160(script))]; ok {
		return nil, ErrDuplicate
	}
...
func (s *Store) ExportWatchingWallet() (*Store, error) {
	...
	ws.keyGenerator = *(kgwc.(*diviAddress))
	...
	for apkh, addr := range s.addrMap {
		if !addr.Imported() {
			// Must be a diviAddress if !imported.
			diviAddr := addr.(*diviAddress)

			ws.chainIdxMap[diviAddr.chainIndex] =
				addr.Address()
		}
		apkhCopy := apkh
...
type WalletAddress interface {
	// Address returns a diviutil.Address for the backing address.
	Address() diviutil.Address
...
func (s *Store) ActiveAddresses() map[diviutil.Address]WalletAddress {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	addrs := make(map[diviutil.Address]WalletAddress)
	for i := int64(rootKeyChainIdx); i <= s.highestUsed; i++ {
		a := s.chainIdxMap[i]
...
//
// A slice is returned with the diviutil.Address of each new address.
// The blockchain must be rescanned for these addresses.
func (s *Store) ExtendActiveAddresses(n int) ([]diviutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (af *addrFlags) ReadFrom(r io.Reader) (int64, error) {
	...
	// diviwallet shall refuse to open any unencrypted addresses.  This
...
// that diviwallet fits into an unused space left by Armory's key store file
// format.
...
type PubKeyAddress interface {
	...
	PrivKey() (*diviec.PrivateKey, error)
	// ExportPrivKey exports the WIF private key.
	ExportPrivKey() (*diviutil.WIF, error)
}

// newDiviAddress initializes and returns a new address.  privkey must
// be 32 bytes.  iv must be 16 bytes, or nil (in which case it is
// randomly generated).
func newDiviAddress(wallet *Store, privkey, iv []byte, bs *BlockStamp, compressed bool) (addr *diviAddress, err error) {
	if len(privkey) != 32 {
		return nil, errors.New("private key is not 32 bytes")
	}

	addr, err = newDiviAddressWithoutPrivkey(wallet,
...
// newDiviAddressWithoutPrivkey initializes and returns a new address with an
...
// randomly generated).
func newDiviAddressWithoutPrivkey(s *Store, pubkey, iv []byte, bs *BlockStamp) (addr *diviAddress, err error) {
	var compressed bool
	switch n := len(pubkey); n {
	case diviec.PubKeyBytesLenCompressed:
		compressed = true
	...
	pk, err := diviec.ParsePubKey(pubkey, diviec.S256())
	if err != nil {
		return nil, err
	}

	address, err := diviutil.NewAddressPubKeyHash(diviutil.Hash160(pubkey), s.netParams())
	if err != nil {
		return nil, err
	}
...
// newRootDiviAddress generates a new address, also setting the
// chaincode and chain index to represent this address as a root
// address.
func newRootDiviAddress(s *Store, privKey, iv, chaincode []byte,
	bs *BlockStamp) (addr *diviAddress, err error) {

	if len(chaincode) != 32 {
		return nil, errors.New("chaincode is not 32 bytes")
	}

	// Create new diviAddress with provided inputs.  This will
	// always use a compressed pubkey.
	addr, err = newDiviAddress(s, privKey, iv, bs, true)
	if err != nil {
		return nil, err
	}
...
// unlocked diviAddress.
func (a *diviAddress) verifyKeypairs() error {
...
func (a *diviAddress) ReadFrom(r io.Reader) (n int64, err error) {
	var read int64
	...
	addr, err := diviutil.NewAddressPubKeyHash(pubKeyHash[:], a.store.netParams())
	if err != nil {
		return n, err
	}
	a.address = addr
...
func (a *diviAddress) WriteTo(w io.Writer) (n int64, err error) {
	var written int64
...
func (a *diviAddress) encrypt(key []byte) error {
...
func (a *diviAddress) unlock(key []byte) (privKeyCT []byte, err error) {
	if !a.flags.encrypted {
	...
	x, y := diviec.S256().ScalarBaseMult(privkey)
...
func (a *diviAddress) changeEncryptionKey(oldkey, newkey []byte) error {
...
func (a *diviAddress) SyncStatus() SyncStatus {
	switch {
...
// PubKeyAddress.
func (a *diviAddress) PubKey() *diviec.PublicKey {
	return a.pubKey
}

func (a *diviAddress) pubKeyBytes() []byte {
	if a.Compressed() {
...
func (a *diviAddress) ExportPubKey() string {
...
func (a *diviAddress) PrivKey() (*diviec.PrivateKey, error) {
...
func (a *diviAddress) ExportPrivKey() (*diviutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}
	...
	// elsewhere.
	wif, err := diviutil.NewWIF((*diviec.PrivateKey)(pk), a.store.netParams(),
		a.Compressed())
	if err != nil {
		panic(err)
	}
...
// normal key store.
func (a *diviAddress) watchingCopy(s *Store) walletAddress {
	return &diviAddress{
		store:   s,
		address: a.address,
		flags: addrFlags{
...
func (a *diviAddress) setSyncStatus(s SyncStatus) {
	switch e := s.(type) {
	case Unsynced:
...
func newScriptAddress(s *Store, script []byte, bs *BlockStamp) (addr *scriptAddress, err error) {
	...
	address, err := diviutil.NewAddressScriptHashFromHash(scriptHash, s.netParams())
	if err != nil {
		return nil, err
	}
...
func (sa *scriptAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	address, err := diviutil.NewAddressScriptHashFromHash(scriptHash[:],
		sa.store.netParams())
	if err != nil {
		return n, err
	}
...
// address returns a diviutil.AddressScriptHash for a diviAddress.
func (sa *scriptAddress) Address() diviutil.Address {
	return sa.address
}
...
func (sa *scriptAddress) Addresses() []diviutil.Address {
	return sa.addresses
}
