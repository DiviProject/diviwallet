...
diviwallet will not drop transaction history by itself, as this is something that
...
```
$ cd $GOPATH/src/github.com/DiviProject/diviwallet/cmd/dropwtxmgr
$ go get
```
...
```
$ dropwtxmgr
Database path: /home/username/.diviwallet/mainnet/wallet.db
Drop all diviwallet transaction history? [y/N] y
...
```
$ dropwtxmgr --db ~/.diviwallet/testnet/wallet.db
Database path: /home/username/.diviwallet/testnet/wallet.db
Drop all diviwallet transaction history? [y/N] y
Dropping wtxmgr namespace
```

After dropping transaction history, diviwallet may be restarted and a full rescan
will be triggered to sync the wallet:

```
$ diviwallet
14:05:31 2015-04-13 [INF] DIVIW: No recorded transaction history -- needs full rescan
...
