...
import (
	...
	"github.com/DiviProject/diviwallet/walletdb/migration"
...
// rescan request.
func (w *Wallet) activeData(dbtx walletdb.ReadTx) ([]diviutil.Address, []wtxmgr.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)

	var addrs []diviutil.Address
	err := w.Manager.ForEachActiveAddress(addrmgrNs, func(addr diviutil.Address) error {
...
func newFilterBlocksRequest(batch []wtxmgr.BlockMeta,
	...
	filterReq := &chain.FilterBlocksRequest{
		Blocks:           batch,
		ExternalAddrs:    make(map[waddrmgr.ScopedIndex]diviutil.Address),
		InternalAddrs:    make(map[waddrmgr.ScopedIndex]diviutil.Address),
...
func (w *Wallet) CreateSimpleTx(account uint32, outputs []*wire.TxOut,
	minconf int32, satPerKb diviutil.Amount, dryRun bool) (
...
// account.
func (w *Wallet) AccountAddresses(account uint32) (addrs []diviutil.Address, err error) {
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
// include a UTXO.
func (w *Wallet) CalculateBalance(confirms int32) (diviutil.Amount, error) {
...
type Balances struct {
	Total          diviutil.Amount
	Spendable      diviutil.Amount
	ImmatureReward diviutil.Amount
}
...
// blockchain or divid mempool), the next chained address is returned.
func (w *Wallet) CurrentAddress(account uint32, scope waddrmgr.KeyScope) (diviutil.Address, error) {
	chainClient, err := w.requireChainClient()
	if err != nil {
		return nil, err
	}
	...
	if props != nil {
		err = chainClient.NotifyReceived([]diviutil.Address{addr})
		if err != nil {
			return nil, err
		}
...
func (w *Wallet) PubKeyForAddress(a diviutil.Address) (*diviec.PublicKey, error) {
	var pubKey *diviec.PublicKey
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
// address.
func (w *Wallet) PrivKeyForAddress(a diviutil.Address) (*diviec.PrivateKey, error) {
	var privKey *diviec.PrivateKey
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		managedAddr, err := w.Manager.Address(addrmgrNs, a)
		if err != nil {
...
func (w *Wallet) HaveAddress(a diviutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a diviutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a diviutil.Address) (waddrmgr.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal diviutil.Amount
		...
		// is calculated.
		feeF64 = (outputTotal - debitTotal).ToDIVI()
	}
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*diviutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
func (w *Wallet) Accounts(scope waddrmgr.KeyScope) (*AccountsResult, error) {
	...
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*diviutil.Amount)
...
type AccountBalanceResult struct {
	...
	AccountName    string
	AccountBalance diviutil.Amount
}
...
func (w *Wallet) DumpPrivKeys() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// privkeys.
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr diviutil.Address) error {
...
func (w *Wallet) DumpWIFPrivateKey(addr diviutil.Address) (string, error) {
...
func (w *Wallet) ImportPrivateKey(scope waddrmgr.KeyScope, wif *diviutil.WIF,
	...
	if rescan {
		...
		err := w.chainClient.NotifyReceived([]diviutil.Address{addr})
		if err != nil {
...
func (w *Wallet) NewAddress(account uint32,
	scope waddrmgr.KeyScope) (diviutil.Address, error) {
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	scope waddrmgr.KeyScope) (diviutil.Address, *waddrmgr.AccountProperties, error) {
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32) (diviutil.Address, error) {
...
// address.
func (w *Wallet) TotalReceivedForAddr(addr diviutil.Address, minConf int32) (diviutil.Amount, error) {
	var amount diviutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SendOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, satPerKb diviutil.Amount) (*wire.MsgTx, error) {
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*diviutil.WIF,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr diviutil.Address) (*diviec.PrivateKey, bool, error) {
			...
			getScript := txscript.ScriptClosure(func(addr diviutil.Address) ([]byte, error) {
...
func (w *Wallet) publishTransaction(tx *wire.MsgTx) (*chainhash.Hash, error) {
	...
	// divid node that already has it in their mempool.
	// https://github.com/DiviProject/divid/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L953
	...
	// confirmed to a divid/bitcoind node over RPC.
	// https://github.com/DiviProject/divid/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/rpcserver.go#L3355
	...
	// already confirmed to a divid node over the P2P network.
	// https://github.com/DiviProject/divid/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L1036
	...
	//
	// This error is returned from divid when there is already a transaction
	// not signaling replacement in the mempool that spends one of the
	// referenced outputs.
	// https://github.com/DiviProject/divid/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L591
	case match(err, "already spent"):
		fallthrough

	// This error is returned from divid when a referenced output cannot be
	// found, meaning it etiher has been spent or doesn't exist.
	// https://github.com/DiviProject/divid/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/blockchain/chain.go#L405
	case match(err, "already been spent"):
		fallthrough

	// This error is returned from divid when a transaction is spending
	// either output that is missing or already spent, and orphans aren't
	// allowed.
	// https://github.com/DiviProject/divid/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L1409
	...
	// Returned by divid when replacement transaction was rejected for
	// whatever reason.
	// https://github.com/DiviProject/divid/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L841
	// https://github.com/DiviProject/divid/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L854
	// https://github.com/DiviProject/divid/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L875
	// https://github.com/DiviProject/divid/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L896
	// https://github.com/DiviProject/divid/blob/130ea5bddde33df32b06a1cdb42a6316eb73cff5/mempool/mempool.go#L913
...
// in order to allow applications wrapping diviwallet to store app-specific data
