...
const DefaultRelayFeePerKb diviutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb diviutil.Amount) diviutil.Amount {
	...
	relayFee := diviutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount diviutil.Amount, scriptSize int, relayFeePerKb diviutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb diviutil.Amount) bool {
	...
	return IsDustAmount(diviutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb diviutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > diviutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb diviutil.Amount, txSerializeSize int) diviutil.Amount {
	fee := relayFeePerKb * diviutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > diviutil.MaxSatoshi {
		fee = diviutil.MaxSatoshi
	}
