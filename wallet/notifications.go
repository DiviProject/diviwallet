...
func makeTxSummary(dbtx walletdb.ReadTx, w *Wallet, details *wtxmgr.TxDetails) TransactionSummary {
	...
	var fee diviutil.Amount
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		for _, deb := range details.Debits {
			fee += deb.Amount
		}
		for _, txOut := range details.MsgTx.TxOut {
			fee -= diviutil.Amount(txOut.Value)
		}
	}
...
func totalBalances(dbtx walletdb.ReadTx, w *Wallet, m map[uint32]diviutil.Amount) error {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
...
func flattenBalanceMap(m map[uint32]diviutil.Amount) []AccountBalance {
...
func relevantAccounts(w *Wallet, m map[uint32]diviutil.Amount, txs []TransactionSummary) {
	for _, tx := range txs {
...
func (s *NotificationServer) notifyUnminedTransaction(dbtx walletdb.ReadTx, details *wtxmgr.TxDetails) {
	...
	bals := make(map[uint32]diviutil.Amount)
...
type TransactionSummaryInput struct {
	...
	PreviousAmount  diviutil.Amount
}
