...
func SumOutputValues(outputs []*wire.TxOut) (totalOutput diviutil.Amount) {
	for _, txOut := range outputs {
		totalOutput += diviutil.Amount(txOut.Value)
	}
	return totalOutput
}
...
// InputSourceError.
type InputSource func(target diviutil.Amount) (total diviutil.Amount, inputs []*wire.TxIn,
	inputValues []diviutil.Amount, scripts [][]byte, err error)
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []diviutil.Amount
	TotalInput      diviutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb diviutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []diviutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := diviutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
