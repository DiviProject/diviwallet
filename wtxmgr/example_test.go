...
func ExampleStore_Balance() {
	...
	// Insert a transaction which outputs 10 DIVI unmined and mark the output
	// as a credit.
	...
	// Output:
	// 10 DIVI, 0 DIVI, 0 DIVI
	// 10 DIVI, 10 DIVI, 0 DIVI
	// 10 DIVI, 10 DIVI, 10 DIVI
}
...
func ExampleStore_Rollback() {
	...
	err = walletdb.Update(db, func(tx walletdb.ReadWriteTx) error {
		...
		// Insert a transaction which outputs 10 DIVI in a block at height 100.
...
func Example_basicUsage() {
	...
	// Insert an unmined transaction that outputs 10 DIVI to a wallet address
	// at output 0.
	...
	// outputs.  Mark the second one (5 DIVI) as wallet change.
