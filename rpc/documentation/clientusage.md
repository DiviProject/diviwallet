...
import (
	...
	pb "github.com/DiviProject/diviwallet/rpc/walletrpc"
...
var certificateFile = filepath.Join(diviutil.AppDataDir("diviwallet", false), "rpc.cert")
...
func main() {
	...
	fmt.Println("Spendable balance: ", diviutil.Amount(balanceResponse.Spendable))
}
```
...
```bash
$ protoc -I/path/to/diviwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/diviwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by diviwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.diviwallet/rpc.cert"s;
    }();
...
```PowerShell
PS> & protoc.exe -I \Path\To\diviwallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by diviwallet).
            Environment.SetEnvironmentVariable("GRPC_SSL_CIPHER_SUITES", "HIGH+ECDSA");

            var walletAppData = Portability.LocalAppData(Environment.OSVersion.Platform, "Diviwallet");
...
var certPath = path.join(process.env.HOME, '.diviwallet', 'rpc.cert');
if (os.platform == 'win32') {
    certPath = path.join(process.env.LOCALAPPDATA, 'Diviwallet', 'rpc.cert');
...
```bash
$ protoc -I /path/to/DiviProject/diviwallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    cert_file_path = os.path.join(os.environ['HOME'], '.diviwallet', 'rpc.cert')
    if platform.system() == 'Windows':
        cert_file_path = os.path.join(os.environ['LOCALAPPDATA'], "Diviwallet", "rpc.cert")
